public class Day02 {
    static class Range {
        long start;
        long end;

        Range(long start, long end) {
            this.start = start;
            this.end = end;
        }
    }

    static String input = "4077-5314,527473787-527596071,709-872,2487-3128,6522872-6618473,69137-81535,7276-8396,93812865-93928569,283900-352379,72-83,7373727756-7373754121,41389868-41438993,5757-6921,85-102,2-16,205918-243465,842786811-842935210,578553879-578609405,9881643-10095708,771165-985774,592441-692926,7427694-7538897,977-1245,44435414-44469747,74184149-74342346,433590-529427,19061209-19292668,531980-562808,34094-40289,4148369957-4148478173,67705780-67877150,20-42,8501-10229,1423280262-1423531012,1926-2452,85940-109708,293-351,53-71";
        //static String input = "11-22,95-115,998-1012,1188511880-1188511890,222220-222224,1698522-1698528,446443-446449,38593856-38593862,565653-565659,824824821-824824827,2121212118-2121212124";
    public static void main(String[] args) {

        java.util.List<Range> ranges = new java.util.ArrayList<>();
        // file input logic
        String[] parts = input.split(",");
        for (String part : parts) {
            part = part.trim();
            if (part.isEmpty()) {
                continue;
            }

            String[] bounds = part.split("-");
            if (bounds.length != 2) {
                throw new IllegalArgumentException("Invalid range segment: " + part);
            }

            long start = Long.parseLong(bounds[0].trim());
            long end = Long.parseLong(bounds[1].trim());

            ranges.add(new Range(start, end));
        }

        // main logic problem 1
        long sum = 0;

        for (Range r : ranges) {
            for (long i = r.start; i < r.end; i++) {
                String num = String.valueOf(i);
                if (num.length() % 2 == 0) {
                    if (num.substring(0, num.length() / 2).equals(num.substring(num.length() / 2, num.length()))) {
                        sum += i;
                    }
                }
            }
        }
        System.err.println(sum);

        // main logic problem 2
        sum = 0;

        for (Range r : ranges) {
            for (long i = r.start; i <= r.end; i++) {
                String num = String.valueOf(i);
                String num1 = num + num;
                if (num1.substring(1, num1.length() - 1).contains(num)) {
                    System.err.println(i);
                    sum += i;
                }

            }
        }
        System.err.println(sum);
    }

}

// s = 12341234
// s+s = 1234123412341234
// s[1,len-1] = 23412341234123
// s must be in s[1,len-1] for in order to form the seq.